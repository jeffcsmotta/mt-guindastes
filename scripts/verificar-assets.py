...
srv = http.server.HTTPServer(('127.0.0.1', 8902), handler)
...
paths.update(['clientes/mt-guindastes/' + p for p in
              ['index.html', 'app.js', 'style.css', 'manifest.json', 'proposta.html']])
bad = []
for p in sorted(paths):
    if not p:
        continue
    try:
        code = urllib.request.urlopen('http://127.0.0.1:8902/' + p.replace(os.sep, '/'), timeout=10).status
        if code != 200:
            bad.append((p, code))
